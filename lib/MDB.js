...
//MONGO DB 접속 (이 부분은 설정파일에서 읽어오도록 변경할 것)
...
//console.info('MDB Function callback!!!!!!!!!!!!!!');
    ...
    //조회 조건을 점검.
    }*/ // MODEL 을 Parameter 로 받지 않고, 선언된 전역에서 가져오는 것으로 변경
...
//console.log('CRITERIA >> ['+criteria+']');
        ...
        // #1001 -- START 모델은 Global 에 정의된 것을 가져 옴.
        var MODEL = models(criteria.VATKEY);
        var model = new MODEL();
        var updateModel = {};
        // #1001 -- END
...
//            console.info('INSERT function....');
        ...
        //DB에 있는 값을 update 하는 부분,
            ...
            updateModel = JSON.parse(JSON.stringify(model));
...
//                    console.info('Successfully updated...');
            ...
            //_id 값 또는 key 값이 다른 경우, 무조건 새로 넣기 때문에 _id 는 삭제해 준다.
            if('_id' in updateModel) {
                delete updateModel._id;
            }
