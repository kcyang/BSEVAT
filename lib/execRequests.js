...
//var mg = require('../lib/MDB.js');  #TODO Will be delete this module.
...
exports.get = function(criteria,callback) {
    ...
    if(criteria === null || typeof criteria === 'undefined') {
        ...
        var MODEL = models(criteria.KEY);

        MODEL.findOne(criteria[1], function (err, result_document) {
            if(err){
                console.error('데이터를 가져오지 못했습니다. ',err);
                return;
            }
//            console.log('가져온 데이터 [%s]'+result_document);
