...
exports.getCompany = function(criteria,callback){
    console.log('GET Data of Company');

    nav.getCompany(criteria, function (err, data) {
        callback(err,data);
    });
};
...
//console.log(criteria.VATKEY + '///' + criteria.KEY);
...
//console.log(criteria.VATKEY + '///' + criteria.KEY);
        ...
        mongoModel.remove({ VATKEY: criteria.VATKEY }, function (err, numRemoved) {

            if(err){
                console.error('데이터를 삭제하지 못했습니다. ',err);
                callback(err, 'ERROR');
            }else {
                console.log('%s 건을 삭제하였습니다.',numRemoved);
...
/* Excel 만들기 */
        ...
        var company_name_param = global.companyname.replace(/[^a-z|^A-Z|^0-9]/gi,'');
        /**
...
// 데이터를 키값을 주고, Excel 을 만들어서 넘겨주는 놈.
exports.makefile = function(VATROOTKEY,callback) {
    ...
    if(VATROOTKEY === null || typeof VATROOTKEY === 'undefined') {
        ...
        var _param_keys = VATROOTKEY.KEY;
        var _param_target = JSON.stringify(VATROOTKEY.PARAM);


        console.log('KEYS',_param_keys);
        console.log('TARGET',_param_target);

        var KEY = _param_keys.YEAR+_param_keys.VATQT;
        ...
        var src = spawn('python',[
            ...
            KEY,
            _param_keys.VATCP,
            serverConfig.excel.result_path
        ]);
        ...
        src.on('exit',function(code){
            ...
            console.log("파일을 정상적으로 생성하였습니다."+serverConfig.excel.result_path+_param_keys.VATCP+'.101');
            callback(false,serverConfig.excel.result_path+_param_keys.VATCP+'.101');
        });
