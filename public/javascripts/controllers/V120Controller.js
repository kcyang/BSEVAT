...
'use strict';
/* global angular */
angular.module('V120Ctrl',['ngGrid'])
    .controller('V120Controller',function($scope,$log,$window,VATService,$location,$route,ngDialog){

        //# 상수정의.
        $scope.constants = {
            'VATNO' : 'V120',  //이 화면의 VAT 번호.
            'EMPTY' : 'true'
        };
        ...
        $scope.calc = function(){
            $scope.mg.RE_IM_BO_AMT = Number($scope.mg.RE_CONT_BO_AMT) * 0.025 * 92/365;
            $scope.mg.RE_IM_AMT = Number($scope.mg.RE_IM_BO_AMT) + Number($scope.mg.RE_IM_MONT_AMT);
            $scope.mg.RE_CONT_BO_TOT_AMT = Number($scope.mg.RE_CONT_BO_AMT);
            $scope.mg.RE_CONT_MONT_TOT_AMT = Number($scope.mg.RE_CONT_MONT_AMT);
            $scope.mg.RE_IM_TOT_AMT = Number($scope.mg.RE_IM_AMT);
            $scope.mg.RE_IM_BO_TOT_AMT = Number($scope.mg.RE_IM_BO_AMT);
            $scope.mg.RE_IM_MONT_TOT_AMT = Number($scope.mg.RE_IM_MONT_AMT);
        };
        ...
        $scope.getExcel = function(){
            ...
            VATService.update($scope.constants.VATNO,$scope.mg,function(err,data){
                ...
                VATService.excel($scope.VATROOTKEY[0],function(err,data){
                     ...
                     *
                     */
                    saveAs(new Blob([data],{type:"application/vnd.ms-excel;charset=euc-kr"}), "부동산임대공금가액명세.xlsx");
