...
'use strict';
/* global angular */
angular.module('V149Ctrl',['ngGrid'])
    .controller('V149Controller',function($scope,$log,$window,VATService,$location,$route,ngDialog){

    //# 상수정의.
    $scope.constants = {
        'VATNO' : 'V149',  //이 화면의 VAT 번호.
        'EMPTY' : 'true'
    };
    ...
    $scope.calc = function(){
        $scope.mg.TOT_COUNT = Number($scope.mg.BID_COUNT) + Number($scope.mg.MAC_COUNT)
        + Number($scope.mg.CAR_COUNT) + Number($scope.mg.ETC_COUNT);
        $scope.mg.TOT_AMOUNT = Number($scope.mg.BID_AMOUNT) + Number($scope.mg.MAC_AMOUNT)
        + Number($scope.mg.CAR_AMOUNT) + Number($scope.mg.ETC_AMOUNT);
        $scope.mg.TOT_TAX = Number($scope.mg.BID_TAX) + Number($scope.mg.MAC_TAX)
        + Number($scope.mg.CAR_TAX) + Number($scope.mg.ETC_TAX);
    };
    ...
    $scope.getExcel = function(){
        ...
        VATService.update($scope.constants.VATNO,$scope.mg,function(err,data){
            ...
            VATService.excel($scope.VATROOTKEY[0],function(err,data){
                 ...
                 *
                 */
                saveAs(new Blob([data],{type:"application/vnd.ms-excel;charset=euc-kr"}), "건물등감가상각자산취득명세서.xlsx");
