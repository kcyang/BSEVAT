...
'use strict';
/* global angular */
angular.module('V177Ctrl',[])
    .controller('V177Controller',function($scope,$log,$window,VATService,$location,$route,ngDialog){

    //# 상수정의.
    $scope.constants = {
        'VATNO' : 'V177',  //이 화면의 VAT 번호.
        'EMPTY' : 'true'
    };
    ...
    $scope.calc = function(){
        $scope.mg.e_a = Number($scope.mg.a_a) + Number($scope.mg.a_b) + Number($scope.mg.a_d) + Number($scope.mg.a_e) +
                         ...
                         Number($scope.mg.d_f) + Number($scope.mg.d_g) + Number($scope.mg.d_h) + Number($scope.mg.a_c);
        $scope.mg.g_a = Number($scope.mg.f_a) + Number($scope.mg.f_b) + Number($scope.mg.f_c) + Number($scope.mg.f_d) +
