...
'use strict';
/* global angular */
angular.module('V174Ctrl',['ngGrid'])
    .controller('V174Controller',function($scope,$log,$window,VATService,$location,$route,ngDialog){

    //# 상수정의.
    $scope.constants = {
        'VATNO' : 'V174',  //이 화면의 VAT 번호.
        'EMPTY' : 'true'
    };
    ...
    $scope.calc = function(){
        $scope.mg.TOT_COUNT = Number($scope.mg.LC_COUNT) + Number($scope.mg.PUR_COUNT) ;
        $scope.mg.TOT_AMOUNT = Number($scope.mg.LC_AMOUNT) + Number($scope.mg.PUR_AMOUNT);
    ...
    $scope.getExcel = function(){
        ...
        VATService.update($scope.constants.VATNO,$scope.mg,function(err,data){
            ...
            VATService.excel($scope.VATROOTKEY[0],function(err,data){
                 ...
                 *
                 */
                saveAs(new Blob([data],{type:"application/vnd.ms-excel;charset=euc-kr"}), "내국신용장_구매확인서 전자발급명세서.xlsx");
...
//TODO 여기서부터 시작합시다...2014-12-19
    $scope.gridOptions = {
        ...
        columnDefs: [
            ...
            {field:'LINE_AMOUNT', displayName:'금액(원)', cellFilter:'number:0', cellClass:'price', enableCellEdit: false},
            {field:'LINE_REMARK', displayName:'비고'}
        ]
    };
});